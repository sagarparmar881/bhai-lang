...
export default class IfStatement implements Visitor {
  ...
  visitNode(node: ASTNode) {
    ...
    if (test) {
      ...
      if (testResult === true || testResult === "sachu") {
        ...
        if (alternates && alternates.length > 0) {
          for (var alternate of alternates) {
            ...
            if (!alternateTest) {
              // Reached the "nahi toh" node in the alternate list, simply evaluate it and break
              this.evaluateNode(alternate, parentScope);
              break;
            } else {
              // Evaluate the "test" condition of the "athva toh" node
