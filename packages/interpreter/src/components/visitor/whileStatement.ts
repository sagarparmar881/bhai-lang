...
export default class WhileStatement implements Visitor {
  visitNode(node: ASTNode) {
    ...
    if (test) {
      ...
      for (let testResult = getConditionValue(), executions = 0; testResult === true || testResult === "sachu"; testResult = getConditionValue(), executions++) {
        ...
        if (executions > 5000) {
          throw new RuntimeException("Daffod, aatli bahi loop kon ferave!");
        }
