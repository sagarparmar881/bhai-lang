...
export function getOperationValue(
  ...
  const exception = new RuntimeException(
    `Oye Babuchak: "${operator}" ke sath "${typeof operands.left}" aur "${typeof operands.right}" nahi jamte.`
  );
  ...
  switch (operator) {
    ...
    case "/":
      if (operands.right === 0) {
        throw new RuntimeException(`Oye Babuchak, aa su kare che??...zero se divide ni karte`);
      }
